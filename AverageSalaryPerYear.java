...
@Override
	public void map(LongWritable key, Text value, Context c) throws IOException, InterruptedException{
		...
		if(!strList[0].equals("serialno")){
			//Fetching the value of the year, state, gender and wage.
			...
			if(wage.matches("^[0-9]+$")){
				c.write(new Text(year+"_"+ state + "_"+ gender),new Text(wage));
			}
		}
	}
}
...
InterruptedException{
	...
	Job j2 = new Job(conf);
	j2.setJobName("Salary Average Per Year job");
	j2.setJarByClass(AverageSalaryPerYear.class);
